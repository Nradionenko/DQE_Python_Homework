            ...
            for key in json.keys():  # json keys = arrays names, in our case - News, Ad or Recipe
                ...
                for d in json[key]:  # each section in json has list of dictionaries with section properties (i.e. News has city adn text etc)
                    if key == label1:  # label1 = News, label2 = ad, label3 = Recipe. Configured in configs.ini
                        ...
                        final_text += com.get_ad(d[txt], d[date])+'\n\n'
                    elif key == label3:
                        final_text += com.get_recipe(d[txt], d[cal])+'\n\n'
        except (KeyError, AttributeError) as e:
            print(e)
        return self.normalize(final_text.rstrip(), strategy)  # normalize method is inherited from fromfile.py and from Functions_Strings_Homework4.py

    def json_full_flow(self):
        raw_source = self.parse()
        my_json, proper_source = self.read_source(raw_source)  # this method is inherited from FromFile class in fromfile.py. It validates file and structure and asks user to input path until proper file is provided
        text = self.process_json(my_json)  # extract values, format them for target doc
